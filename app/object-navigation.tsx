...
const ObjectDetectionScreen = () => {
  ...
  const buildSpeech = (det: any) => {
    ...
    return `${det.class} ${distanceMap[det.distance]} ${positionMap[det.position]}`;
  };
  ...
  const captureObjects = async () => {
    ...
    try {
      ...
      if (result.detections && result.detections.length > 0) {
        ...
        const speech = buildSpeech(topDetection);

        if (lastDetection !== speech) {
          setLastDetection(speech);
          speak?.(speech, true);
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={[styles.statusOverlay, { backgroundColor: colors.card }]}>
          ...
          <TouchableOpacity onPress={checkConnection}>
            <AccessibleText style={styles.refreshIcon}>🔄</AccessibleText>
          </TouchableOpacity>
        </View>
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <View style={[styles.topBar, { backgroundColor: colors.primary }]}>
        <AccessibleText style={[styles.title, { color: colors.textInverse }]} level={1}>
          Object Detection
        </AccessibleText>
      </View>
...
const styles = StyleSheet.create({
  ...
  permissionButton: { paddingHorizontal: 24, paddingVertical: 16, borderRadius: 16 },
  statusOverlay: {
    position: "absolute",
    top: 20,
    left: 20,
